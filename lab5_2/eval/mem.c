...
{   
    ...
    printf("Page Size = %d.\n", pageSize);
    sizeOfRegion += sizeof(struct __list_t);		// add extra bytes for head-node (??)
    ...
    memInitialized = true;
    printf("Successfully Initialized %d (+%d) Bytes of Memory (8-Byte Aligned).\n", memHead->size, (int)sizeof(struct __list_t));
...
void *Mem_Alloc(int size, int style)
{
    ...
    else if(style == BESTFIT)
    {
        ...
        MemNodePtr bestFree = NULL;
        while(tmp!=NULL)
        {
            if (tmp->free && tmp->size >= size)
            {
                if (bestFree == NULL || tmp->size < bestFree->size)
                    bestFree = tmp;
            }
            tmp = tmp->next;
        } 
